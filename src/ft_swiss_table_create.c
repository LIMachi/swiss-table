											...
											t_swt_cmpfun cmp)
{
	...
	out = (t_swt_map){.pair_count = 0, .nb_groups = SWT_GROUP_SIZE,
		.hashfun = hash, .cmpfun = cmp,
		.groups = malloc(sizeof(t_swt_group) * SWT_GROUP_SIZE),
		.values = malloc(sizeof(SWT_VALUE_TYPE) * SWT_VALUE_SIZE)};
											...
											t_swt_cmpfun cmp)
{
	...
	out = (t_swt_map){.pair_count = 0, .nb_groups = SWT_GROUP_SIZE,
		.hashfun = hash, .cmpfun = cmp,
		.groups = malloc(sizeof(t_swt_group) * SWT_GROUP_SIZE),
		.values = malloc(sizeof(SWT_KEY_TYPE) * SWT_VALUE_SIZE)};
	if (out.groups == NULL || out.values == NULL)
	{
		free(out.groups);
		free(out.values);
